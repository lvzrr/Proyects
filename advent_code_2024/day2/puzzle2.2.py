...
def check_remove_level(row,increasing):
    for i in range(0,len(row)):
        temp = row.copy()
        temp.remove(temp[i])
        if check_diff(temp,increasing) and check_increments(temp,increasing):
            print(row,"->",temp,"| Dampened:",i)
            return True
    return False
...
for row in data:
    ...
    if check_diff(row,increasing) and check_increments(row,increasing):
        ...
        if check_remove_level(row,increasing):
            out+=1
print(out)
